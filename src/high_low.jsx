...
export const HighLow = () => {
  ...
  const [previousCard, setPreviousCard] = useState(null); // 前回のカード
  ...
  const [pendingGuess, setPendingGuess] = useState(null); // プレイヤーの予想 ('high' または 'low')
  ...
  const handleGuess = (guess) => {
    ...
    setPendingGuess(guess); // プレイヤーの予想を保存
      ...
      {gameState === 'betting' && (
        <form onSubmit={handleBetSubmit} className="flex items-center justify-center space-x-4 mb-4">
          ...
          <input
            ...
            max={chips}
          />
          <button type="submit" className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-4 rounded shadow-lg">
            カードを引く
          </button>
        </form>
      )}
